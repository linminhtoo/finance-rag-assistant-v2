...
def _print_record(
    ...
    *,
    ...
    out = {
        ...
        "context": _maybe_truncate(_extract_context(payload, context_metadata_key=context_metadata_key) or "", max_chars=max_chars),
        "text": _maybe_truncate(payload.get("text") or "", max_chars=max_chars),
        "index_text": _maybe_truncate((metadata.get("index_text") or payload.get("index_text") or "not found"), max_chars=max_chars),
    }
    if as_json:
    ...
    if chunk_id:
        try:
            records = client.get(
                ...
                ids=[chunk_id],
                output_fields=["chunk_id", "payload"],
            )
        except TypeError:
            records = client.get(
                collection_name,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-chars", type=int, default=400, help="Truncate context to this many chars (use 0 to disable).")
